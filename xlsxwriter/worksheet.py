...
class Worksheet(xmlwriter.XMLwriter):
    ...
    def _write_string(self, row, col, string, cell_format=None):
        ...
        if self.constant_memory and row > self.previous_row:
            self._write_constant_memory_row(row)
    ...
    def _write_array_formula(self, first_row, first_col, last_row, last_col,
        ...
        if self.constant_memory and first_row > self.previous_row:
            self._write_constant_memory_row(first_row)
    ...
    def _write_constant_memory_row(self, current_row_num):
        # current_row_num is always >= self.previous_row
        if (current_row_num-self.previous_row) < self.constant_memory_row_buffer:
            # Write the row buffer only after it has been filled
            return

        self._write_constant_memory_row_buffer(current_row_num)

    def _write_remaining_constant_memory_row_buffer(self):
        # Used by the packager to write any rows left in the buffer
        if len(self.table) > 0:
            self._write_constant_memory_row_buffer(sorted(self.table.keys())[-1]+1)

    def _write_constant_memory_row_buffer(self, current_row_num):
        ...
        # specified by the constant_memory_row_buffer property are kept
